...
@app.post("/gs/")
async def create_item(group_a: Participants, 
                      group_b: Participants, 
                      pref_a: Preferences, 
                      pref_b: Preferences):
    # return {"group_a": group_a.data, "pref_a": pref_a.data}
    gs_alg = GaleShapley()
    result = gs_alg.match(group_a.data, group_b.data, pref_a.data, pref_b.data)
    return {*result}